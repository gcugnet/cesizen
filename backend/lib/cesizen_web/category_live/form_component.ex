defmodule CesizenWeb.CategoryLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      <.header>
        {@title}
        <:subtitle>
          Use this form to manage category records in your database.
        </:subtitle>
      </.header>
    ...
    case AshPhoenix.Form.submit(socket.assigns.form, params: category_params) do
      {:ok, category} ->
        ...
        socket =
          ...
          |> put_flash(
            :info,
            "Category #{socket.assigns.form.source.type}d successfully"
          )
  ...
  defp assign_form(%{assigns: %{category: category}} = socket) do
    form =
      if category do
        ...
        AshPhoenix.Form.for_create(Cesizen.Information.Category, :create,
          as: "category",
          actor: socket.assigns.current_user
        )
      end
